        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="space-y-6">
                ...
                <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
                    <div>
                        ...
                        <p class="text-xs text-slate-500 mt-0.5">Gestión y actualización de entrega de activos</p>
                    </div>

                    {{-- Formulario para actualizar el estado de credencial y uniforme --}}
                    <form action="{{ route('admin.desvinculacion.update-talento-humano', $solicitud->id) }}"
                        ...
                        <!-- Credencial -->
                        <div
                            class="flex items-center justify-between p-3 rounded-lg border text-xs {{ $solicitud->devolver_credencial ? 'bg-emerald-50/50 border-emerald-200 text-emerald-900' : 'bg-amber-50/50 border-amber-200 text-amber-900' }}">
                            <div class="flex items-center gap-2">
                                <input type="checkbox" name="devolver_credencial" id="devolver_credencial"
                                    value="1" {{ $solicitud->devolver_credencial ? 'checked' : '' }}
                                    class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 h-4 w-4">
                                <label for="devolver_credencial" class="font-medium cursor-pointer select-none">
                                    Entregar Credencial
                                </label>
                            </div>
                            <span class="font-bold inline-flex items-center gap-1">
                                <i
                                    class="fas {{ $solicitud->devolver_credencial ? 'fa-check text-emerald-600' : 'fa-clock text-amber-600' }}"></i>
                                {{ $solicitud->devolver_credencial ? 'Entregada' : 'Pendiente' }}
                            </span>
                        </div>

                        <!-- Uniformes -->
                        <div
                            class="flex items-center justify-between p-3 rounded-lg border text-xs {{ $solicitud->devolver_uniforme ? 'bg-emerald-50/50 border-emerald-200 text-emerald-900' : 'bg-amber-50/50 border-amber-200 text-amber-900' }}">
                            <div class="flex items-center gap-2">
                                <input type="checkbox" name="devolver_uniforme" id="devolver_uniforme" value="1"
                                    {{ $solicitud->devolver_uniforme ? 'checked' : '' }}
                                    class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 h-4 w-4">
                                <label for="devolver_uniforme" class="font-medium cursor-pointer select-none">
                                    Entregar Uniformes
                                </label>
                            </div>
                            <span class="font-bold inline-flex items-center gap-1">
                                <i
                                    class="fas {{ $solicitud->devolver_uniforme ? 'fa-check text-emerald-600' : 'fa-clock text-amber-600' }}"></i>
                                {{ $solicitud->devolver_uniforme ? 'Entregados' : 'Pendiente' }}
                            </span>
                        </div>
                        ...
                        <div class="pt-2 flex justify-end">
                            <button type="submit"
                                class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow-sm transition">
                ...
                @if ($solicitud->observaciones)
                    <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-2">
                        ...
                        <p
                            class="text-xs text-slate-700 bg-slate-50 p-3 rounded-lg border border-slate-100 leading-relaxed">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
                    ...
                    @if ($equiposAsignados->count())
                        <div class="space-y-3">
                            @foreach ($equiposAsignados as $equipo)
                                ...
                                <div
                                    class="p-4 border border-slate-200 rounded-xl bg-white hover:border-slate-300 transition flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    <div class="space-y-1">
                                        ...
                                        <p class="text-xs text-slate-500">
                                            Código/Serie: <span
                                                class="font-mono text-slate-700 font-medium">{{ $equipo->codigo ?? ($equipo->serie ?? 'N/A') }}</span>
                                        </p>

                                        @if ($observacionEquipo)
                                            <p
                                                class="text-xs text-slate-600 bg-slate-50 border border-slate-200 p-2 rounded-md mt-2 italic flex items-start gap-1.5">
                    ...
                    @else
                        <div
                            class="p-8 text-center border border-dashed border-slate-200 rounded-xl bg-slate-50 text-slate-500 text-xs">
                ...
                <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
                    ...
                    @if ($solicitud->comprobante)
                        <div class="border border-slate-200 rounded-xl overflow-hidden bg-slate-50">
                            <div
                                class="flex items-center justify-between px-4 py-2.5 bg-slate-100/80 border-b border-slate-200">
