...
<div class="max-w-4xl mx-auto bg-white shadow-xl rounded-2xl p-8">
    ...
    <form
        x-data="formIndumentariaEdit()"
        action="{{ route('admin.indumentarias.update', $indumentaria) }}"
        method="POST"
    ...
    >
        ...
        <div>
            ...
            <div class="flex items-center gap-6">
                ...
                <div class="w-40 h-40 border-2 border-dashed rounded-xl overflow-hidden bg-gray-50 flex items-center justify-center">

                    <img x-show="imageUrl"
                         :src="imageUrl"
                         class="w-full h-full object-cover">

                    <span x-show="!imageUrl"
        ...
        <div>
            ...
            <div class="space-y-4">
                ...
                @foreach ($ubicaciones as $ubicacion)
                    ...
                    @php
                        $inventarioNuevo = $indumentaria->inventarios
                            ->firstWhere('ubicacion_id', $ubicacion->id);

                        $inventarioUsado = $indumentaria->inventariosUsados
                            ->firstWhere('ubicacion_id', $ubicacion->id);

                        $ubicacionesActuales = $indumentaria->inventarios
                            ->pluck('ubicacion_id')
                            ->toArray();
                    @endphp
...
function formIndumentariaEdit() {
    return {
        ...
        ubicacionesSeleccionadas: @json(
            old(
                'ubicaciones',
                $indumentaria->inventarios->pluck('ubicacion_id')
            )
        ),
