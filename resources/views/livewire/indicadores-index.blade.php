<div x-data="{ showForm: false }" x-on:close-form.window="showForm = false" class="space-y-6 max-w-7xl mx-auto">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            ...
                            @if ($indicadores && count($indicadores) > 0)
                                <span
                                    class="bg-indigo-100 text-indigo-700 text-xs font-semibold px-2.5 py-0.5 rounded-full">
                                    {{ count($indicadores) }}
                                </span>
                            @endif
                        </div>
        ...
        <div class="p-6">
            @if ($indicadores && count($indicadores) > 0)
                <div class="grid grid-cols-1 gap-4">
                    @foreach ($indicadores as $indicador)
                        <div wire:key="indicador-{{ $indicador->id }}"
                            ...
                            <div class="flex items-start sm:items-center gap-4">
                                <div
                                    class="bg-indigo-50 text-indigo-600 p-3 rounded-xl group-hover:bg-indigo-100 transition-colors shrink-0">
                                    <i class="fa-solid fa-chart-line text-lg"></i>
                                </div>
                                <div>
                                    <h3
                                        class="font-semibold text-gray-900 text-base sm:text-lg group-hover:text-indigo-900 transition-colors">
                                        {{ $indicador->nombre }}
                                    </h3>
                                    @if (isset($indicador->frecuencia))
                                        <span class="inline-flex items-center text-xs text-gray-500 mt-1 gap-1">
                                            <i class="far fa-clock"></i> Frecuencia:
                            ...
                            {{-- Acciones --}}
                            <div
                                class="flex items-center justify-end gap-1.5 pt-3 sm:pt-0 border-t sm:border-t-0 border-gray-100">
                                ...
                                @role('Admin')
                                    <!-- Editar -->
                                    <button @click="showForm = true" wire:click="editIndicador({{ $indicador->id }})"
                                    ...
                                    <!-- Eliminar -->
                                    <button wire:click="destroyIndicador({{ $indicador->id }})"
            ...
            @else
                ...
                <div class="text-center py-16 px-4">
                    <div
                        class="inline-flex items-center justify-center w-20 h-20 bg-indigo-50 text-indigo-400 rounded-full mb-4 shadow-inner">
    ...
    <div x-show="showForm" x-transition:enter="transition ease-out duration-300"
        ...
        class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden" style="display: none;">
        ...
        <div class="px-6 py-5 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
            ...
            <button type="button" @click="showForm = false; $wire.call('resetForm')"
        ...
        {{-- Formulario --}}
        <form wire:submit="{{ $editingIndicadorId ? 'update' : 'store' }}" class="p-6 space-y-6">

            <div class="space-y-4">
                {{-- Nombre --}}
                <div>
                    <x-label for="nombre" value="Nombre del Indicador" class="font-semibold text-gray-700 mb-1" />
                    <x-input id="nombre" wire:model="nombre"
                ...
                <div>
                    ...
                    <textarea wire:model="forma_calculo" rows="4"
                ...
                {{-- Sentido / Orientación de la Meta --}}
                <div>
                    <x-label value="Sentido / Comportamiento deseado" class="font-semibold text-gray-700 mb-1" />
                    <select wire:model="sentido"
                        class="w-full border-gray-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 shadow-sm text-sm bg-white">
                        <option value="ascendente">⬆️ Ascendente (A mayor resultado, mejor)
                        </option>
                        <option value="descendente">⬇️ Descendente (A menor resultado, mejor)</option>
                        <option value="mantenimiento">🎯 Mantenimiento (El resultado debe ser exacto a la meta)</option>
                    </select>
                    <x-input-error for="sentido" class="mt-1" />
                </div>
                {{-- Frecuencia --}}
                <div>
                    <x-label value="Frecuencia de Medición" class="font-semibold text-gray-700 mb-1" />
                    <select wire:model="frecuencia_id"
