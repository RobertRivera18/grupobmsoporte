<div class="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto space-y-6">
    ...
    {{-- Encabezado --}}
    <div
        class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-gray-200 dark:border-gray-800">
        <div>
    ...
    @if (session()->has('error'))
        <div
            class="p-4 bg-rose-50 dark:bg-rose-950/60 text-rose-600 dark:text-rose-400 rounded-xl text-sm border border-rose-200 dark:border-rose-800">
    ...
    @if ($isFinished && !$showReview)
        <div
            class="bg-white dark:bg-gray-900 rounded-2xl p-6 sm:p-8 border border-gray-200 dark:border-gray-800 space-y-6 shadow-sm">

            {{-- Resumen de Nota Prevaleciente --}}
            <div class="text-center space-y-4">
                <div
                    class="inline-flex items-center justify-center w-16 h-16 rounded-full {{ $passed ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-950/60 dark:text-emerald-400' : 'bg-rose-100 text-rose-600 dark:bg-rose-950/60 dark:text-rose-400' }}">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        ...
                        <span class="font-bold text-sm text-gray-900 dark:text-white">{{ $percentage }}%</span>
                        ({{ $score }} pts)
                    </p>
                </div>
            ...
            <div class="pt-6 border-t border-gray-100 dark:border-gray-800">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-xs text-gray-500 dark:text-gray-400 font-medium">
                        Intentos realizados: {{ $attempts->count() }} @if ($quiz->max_attempts)
                            / {{ $quiz->max_attempts }}
                        @endif
                    </span>
                </div>
                            ...
                            @foreach ($attempts as $index => $att)
                                ...
                                <tr>
                                    <td class="py-3 font-semibold text-gray-900 dark:text-white">
                                        ...
                                        @if ($isBest)
                                            <span
                                                class="ml-1 text-[10px] font-medium bg-amber-100 text-amber-800 dark:bg-amber-950/60 dark:text-amber-400 px-1.5 py-0.5 rounded">Nota
                                                Más Alta</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="py-3 text-center">
                                        <span
                                            class="px-2 py-0.5 rounded-full text-[10px] font-semibold {{ $att->passed ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-400' : 'bg-rose-100 text-rose-700 dark:bg-rose-950/60 dark:text-rose-400' }}">
                                            {{ $att->passed ? 'Aprobado' : 'No Aprobado' }}
                                        </span>
                                    </td>
                                    <td class="py-3 text-right">
                                        <button type="button" wire:click="reviewAttempt({{ $att->id }})"
    ...
    @else
        ...
        @if ($currentQuestion)
            @php
                $qType =
                    $currentQuestion->question_type instanceof \BackedEnum
            ...
            <div
                class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 space-y-6 shadow-sm">

                {{-- Encabezado de la pregunta --}}
                <div class="flex items-center justify-between pb-4 border-b border-gray-100 dark:border-gray-800">
                    <span
                        class="text-xs font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/50 px-2.5 py-1 rounded-lg">
                        Pregunta {{ $currentQuestionIndex + 1 }} de {{ $quiz->questions->count() }}
                    </span>
                    <span class="text-xs text-gray-400 font-medium">
                        {{ $isMultiple ? 'Selección Múltiple' : 'Selección Única' }} •
                        {{ $currentQuestion->pivot->points ?? ($currentQuestion->points ?? 1) }} Ptos
                    </span>
                </div>
                ...
                <div class="space-y-2.5">
                    @foreach ($currentQuestion->options as $option)
                        ...
                        <div @if (!$isFinished) wire:click="selectOption({{ $currentQuestion->id }}, {{ $option->id }})" @endif
                            ...
                            @else
                                <input type="radio" name="q_{{ $currentQuestion->id }}" @checked($isSelected)
                                    @disabled($isFinished)
                ...
                @if ($showReview && !empty($currentQuestion->explanation))
                    <div
                        class="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 text-xs text-amber-800 dark:text-amber-300">
                        <i class="fas fa-lightbulb mr-1"></i> <strong>Explicación:</strong>
                ...
                <div class="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-800">
                    <button type="button" wire:click="previousQuestion" @disabled($currentQuestionIndex === 0)
