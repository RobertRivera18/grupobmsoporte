<div class="space-y-6 max-w-4xl mx-auto mt-8">
    ...
    <div class="relative group">
        <span
            class="absolute inset-y-0 left-4 flex items-center text-gray-400 group-focus-within:text-gray-900 transition-colors">
    ...
    @if ($search && $this->colaboradores->count())
        <ul
            class="absolute z-30 w-full bg-white/95 backdrop-blur-md border border-gray-100 rounded-2xl shadow-xl max-h-60 overflow-auto divide-y divide-gray-50 mt-1">
            @foreach ($this->colaboradores as $user)
                <li wire:click="seleccionarColaborador({{ $user->id }})"
                    ...
                    <span
                        class="text-xs text-gray-400 font-mono bg-gray-50 px-2 py-1 rounded-md group-hover:bg-white transition-colors">C.I.
                        {{ $user->cedula }}</span>
                </li>
            @endforeach
        </ul>
        ...
        <div class="bg-white border border-gray-200/80 rounded-3xl p-7 space-y-6 shadow-sm">
            <div class="flex items-center justify-between pb-5 border-b border-gray-100">
                <div class="flex items-center gap-4">
                    <div
                        class="w-12 h-12 rounded-2xl bg-gray-900 text-white flex items-center justify-center font-semibold text-sm shadow-md shadow-gray-900/10">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 font-medium mt-0.5">Cédula: <span
                                class="font-mono text-gray-700">{{ $this->colaborador->cedula }}</span></p>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div class="bg-gray-50/60 border border-gray-100 rounded-2xl p-4 flex items-start gap-3.5">
                    <div
                        class="w-8 h-8 rounded-xl bg-white border border-gray-200/60 flex items-center justify-center text-gray-600 shrink-0 shadow-xs">
                        <i class="fas fa-users text-xs"></i>
                    </div>
                    <div>
                        <span class="block text-[10px] font-bold uppercase tracking-wider text-gray-400">Cuadrilla
                            Asignada</span>
                ...
                <div class="bg-gray-50/60 border border-gray-100 rounded-2xl p-4 flex items-start gap-3.5">
                    ...
                    <div>
                        <span class="block text-[10px] font-bold uppercase tracking-wider text-gray-400">Compañero de
                            Cuadrilla</span>
                        <span class="text-xs font-bold text-gray-800 mt-0.5 block">
            ...
            <div class="space-y-4 pt-2">
                ...
                @role('Admin|Talento Humano')
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                        <label
                            class="flex items-center gap-3 p-3.5 bg-white border border-gray-200/80 rounded-2xl cursor-pointer hover:border-gray-300 hover:bg-gray-50/50 transition-all select-none">
                            <input type="checkbox" wire:model="devolverCredencial"
                        ...
                        <label
                            class="flex items-center gap-3 p-3.5 bg-white border border-gray-200/80 rounded-2xl cursor-pointer hover:border-gray-300 hover:bg-gray-50/50 transition-all select-none">
                            <input type="checkbox" wire:model="devolverUniforme"
                                class="w-4 h-4 text-gray-900 border-gray-300 rounded focus:ring-gray-900 cursor-pointer">
                ...
                <div class="flex justify-end pt-3">
                    <button type="button" wire:click="guardarSolicitudTTHH"
                        ...
                        <span>Guardar y Enviar a Talento Humano/Sistemas</span>
