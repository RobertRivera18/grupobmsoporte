    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mb-1">
                ...
                <span class="truncate max-w-[150px] sm:max-w-xs">{{ $course->name }}</span>
                <i class="fas fa-chevron-right text-[10px]"></i>
                <span>Editar</span>
            </div>
    ...
    <form wire:submit.prevent="update"
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="w-full">
                        <label
                            class="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed rounded-xl cursor-pointer bg-gray-50/50 dark:bg-gray-800/20 border-gray-300 dark:border-gray-700 hover:bg-gray-100/50 dark:hover:bg-gray-800/40 transition-colors">
                            <div class="flex flex-col items-center justify-center pt-4 pb-5 text-center">
                                ...
                                <p class="text-xs text-gray-600 dark:text-gray-300 font-medium">Reemplazar imagen actual
                                </p>
                    ...
                    <div
                        class="relative w-full h-36 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
                        @if ($image)
                            <!-- Nueva imagen seleccionada en la sesión actual -->
                            <img src="{{ $image->temporaryUrl() }}" class="w-full h-full object-cover"
                                alt="Nueva portada">
                            <span
                                class="absolute top-2 left-2 px-2 py-0.5 text-[10px] font-semibold bg-indigo-600 text-white rounded-md shadow-xs">
                                Nueva imagen
                            </span>
                        @elseif ($currentImage)
                            <!-- Imagen previa registrada en Base de Datos -->
                            <img src="{{ str_starts_with($currentImage, 'http') ? $currentImage : asset('storage/' . $currentImage) }}"
                                class="w-full h-full object-cover" alt="Portada actual"
                                onerror="this.onerror=null; this.src='https://placehold.co/600x400?text=Error+al+cargar+imagen';">
                            <span
                                class="absolute top-2 left-2 px-2 py-0.5 text-[10px] font-semibold bg-gray-900/70 text-white rounded-md shadow-xs backdrop-blur-xs">
                                Actual
                            </span>
                        @else
                            <!-- Sin imagen cargada -->
                        ...
                        <div wire:loading wire:target="image"
                            class="absolute inset-0 bg-gray-900/60 backdrop-blur-xs flex items-center justify-center text-white">
            ...
            <div class="pt-2">
                <label class="flex items-center gap-3 cursor-pointer select-none">
                    ...
                    <div>
                        <span class="text-sm font-semibold text-gray-900 dark:text-white block">Estado del curso</span>
                        <span class="text-xs text-gray-500 dark:text-gray-400 block">Determina si el curso se encuentra
                            activo y visible públicamente.</span>
                    </div>
                </label>
            </div>
        ...
        <div
            ...
            <button type="submit" wire:loading.attr="disabled"
                ...
                <span wire:loading.remove wire:target="update">Actualizar Curso</span>
                <span wire:loading wire:target="update" class="inline-flex items-center gap-2">
                    <i class="fas fa-spinner fa-spin text-xs"></i> Guardando cambios...
                </span>
            </button>
        </div>
